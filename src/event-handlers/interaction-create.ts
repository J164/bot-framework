...
import { type ApplicationCommandHandlers, type BotClient, type ChatInputCommand } from '../bot-client.js';
...
export async function onInteractionCreate(this: BotClient, handlers: ApplicationCommandHandlers, interaction: Interaction): Promise<void> {
	if (interaction.isChatInputCommand()) {
		await handleChatInputCommand(interaction, handlers, this.logger);
	}
}

async function handleChatInputCommand(interaction: ChatInputCommandInteraction, handlers: ApplicationCommandHandlers, logger: Logger): Promise<void> {
